...
module.exports = function(Cam) {
	...
	Cam.prototype.createPullPointSubscription = function(callback) {
		this._request({
			...
			var subscriptionEventData = null;
			
			if (isEmpty(err) && res != "" && res.length && res[0].createPullPointSubscriptionResponse != undefined) {
				this.events.subscription = linerase(res[0].createPullPointSubscriptionResponse[0]);
				
				var address = this.events.subscription.subscriptionReference.address;
				...
				this.events.terminationTime = _terminationTime(this.events.subscription);
				 ...
				 subscriptionEventData = { subscription: address };
			}
			else {
				err = {msg: "something went wrong - custom error message"};
				
				 subscriptionEventData = null;

			}
			
			this.emit('subscriptionEvent', subscriptionEventData);
